 ...
 * representative components, without touching the live site.
 *
 * Two modes:
 *  - Single  — flip through one theme at a time.
 *  - Compare — two themes side by side on identical content (head-to-head).
 *
 * A light/dark toggle applies to whatever is on screen. Not indexed (see
 * metadata in the page). Once a direction is chosen, the winning fonts +
 * palette get promoted into globals.css / layout.tsx site-wide.
 */
...
const THEMES: Theme[] = [
  {
    ...
    light: {
      ...
      primary: "#2f4a7a", primaryFg: "#ffffff", accent: "#9a6510", accentFg: "#ffffff",
    },
    dark: {
  ...
  },
  {
    ...
    light: {
      ...
      primary: "#2c7da0", primaryFg: "#ffffff", accent: "#c2641f", accentFg: "#ffffff",
    },
    dark: {
...
/** The themed sample "site". Renders entirely from the passed theme's CSS vars. */
function ThemeCanvas({
  ...
  const heroH = compact ? "text-3xl md:text-4xl" : "text-4xl md:text-6xl";
  const heroPad = compact ? "py-12" : "py-16 md:py-20";
  const secPad = compact ? "pb-12" : "pb-16";
    ...
    <div style={vars}>
      <div style={{ background: "var(--lab-bg)", color: "var(--lab-ink)" }}>
        ...
        >
          ...
          <div className="hidden xl:flex items-center gap-6 text-sm" style={{ color: "var(--lab-muted)" }}>
        ...
        {/* Hero */}
        <div className={`container mx-auto px-4 md:px-6 ${heroPad} text-center`}>
          <span
          ...
          <h2 className={`${heroH} font-bold tracking-tight max-w-3xl mx-auto`} style={heading}>
            Your Community for{" "}
            <span style={{ color: "var(--lab-accent)" }}>Connection &amp; Growth</span>
          </h2>
          <p className="mt-6 text-lg max-w-2xl mx-auto leading-relaxed" style={{ color: "var(--lab-muted)" }}>
        ...
        {/* Stats */}
        <div className={`container mx-auto px-4 md:px-6 ${secPad}`}>
          <div
        ...
        {/* Membership */}
        <div className={`container mx-auto px-4 md:px-6 ${compact ? "pb-12" : "pb-20"}`}>
...
/** Small label strip shown above each column in compare mode. */
function ColumnHeader({ theme, badge }: { theme: Theme; badge: string }) {
  return (
    <div className="px-4 py-3 border-b border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800">
      <div className="flex items-center gap-2">
        <span
          className="text-[10px] font-bold uppercase tracking-wider rounded px-1.5 py-0.5 bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900"
        >
          {badge}
        </span>
        <span className="flex gap-1">
          <span className="w-3.5 h-3.5 rounded-full" style={{ background: theme.light.primary }} />
          <span className="w-3.5 h-3.5 rounded-full" style={{ background: theme.light.accent }} />
        </span>
        <span className="font-bold text-zinc-900 dark:text-zinc-50" style={{ fontFamily: theme.headingFont }}>
          {theme.name}
        </span>
      </div>
      <span className="block text-[11px] text-zinc-500 mt-0.5">
        {theme.headingLabel} + {theme.bodyLabel}
      </span>
    </div>
  );
}
...
export function StyleLab() {
  ...
  const [leftIndex, setLeftIndex] = useState(0); // Hill Country
  const [rightIndex, setRightIndex] = useState(2); // Warm Welcome
  ...
  const btn = (active: boolean) =>
    `inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
      active
        ...
        : "border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-200 dark:hover:bg-zinc-800"
    }`;

  const selectCls =
    "rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 px-3 py-2 text-sm font-medium text-zinc-800 dark:text-zinc-100";
  ...
  return (
    <div className="min-h-screen bg-zinc-100 dark:bg-zinc-900">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs text-zinc-500">
                {mode === "compare"
                  ? "Head-to-head: two themes on identical content. Scroll — both move together."
                  : "Pick a direction, then tell Claude the name to apply it site-wide."}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button className={btn(mode === "single")} onClick={() => setMode("single")}>
                <Square className="w-4 h-4" /> Single
              </button>
              <button className={btn(mode === "compare")} onClick={() => setMode("compare")}>
                <Columns2 className="w-4 h-4" /> Compare
              </button>
              <button className={btn(false)} onClick={() => setIsDark((d) => !d)}>
      ...
      {/* ---------- Preview ---------- */}
      {mode === "single" ? (
        <ThemeCanvas theme={THEMES[themeIndex]} isDark={isDark} />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 lg:divide-x divide-zinc-300 dark:divide-zinc-700">
          <div className="min-w-0">
            <ColumnHeader theme={THEMES[leftIndex]} badge="A" />
            <ThemeCanvas theme={THEMES[leftIndex]} isDark={isDark} compact />
          </div>
          <div className="min-w-0 border-t lg:border-t-0 border-zinc-300 dark:border-zinc-700">
            <ColumnHeader theme={THEMES[rightIndex]} badge="B" />
            <ThemeCanvas theme={THEMES[rightIndex]} isDark={isDark} compact />
          </div>
        </div>
      )}
    </div>
